...
A session whose worktree directory belongs to no git repository, because the repository was deleted or a [`WorktreeCreate` hook](/docs/en/hooks#worktreecreate) created the directory elsewhere, can still be deleted. When files remain, agent view asks for the same `Ctrl+X` double-press before discarding them; for a hook-created directory it runs your [`WorktreeRemove` hook](/docs/en/hooks#worktreeremove) instead, and without one it refuses the delete and keeps the session. For a directory that still has files, `claude rm` keeps the session and worktree instead and names the reason. Either path keeps a directory that another finished session's records name.
