...
![](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/%E7%99%BB%E5%BD%95%E5%A2%9E%E5%BC%BAwork.DWixAFfl.png)- **常见原因(2\)**：用户没有WorkBuddy账号文件夹操作权限。
- **建议处理**
...
### 1）客户端出现异常，如何快速自检（诊断工具）

- **适用系统**：`Windows`、`macOS` 桌面端。
- **功能说明**：**WorkBuddy** 内置「诊断工具」，可一键检测网络、系统资源、安全软件、环境信息与崩溃日志，结果可视化展示，并自动生成诊断报告。遇到无法登录、连接失败、客户端异常等问题时，建议先运行自检。
- **建议处理**：

	1. 打开 **WorkBuddy**，点击顶部**帮助 → 诊断工具**，弹出诊断窗口；
	2. 点击**开始检测**，逐项执行检查，实时显示进度与结果；
	3. 检测过程中关闭弹窗不影响检测进行，重新打开可恢复查看进度与结果；
	4. 检测完成后自动生成 `.txt` 纯文本报告，保存在日志目录 `logs/self-check/` 下（保留最近 10 份），可在弹窗底部点击**导出报告**或**定位文件**获取，提交反馈时一并提供。
- **检查项说明**：

| 大类 | 检查内容 |
| --- | --- |
| 网络 | API 连通性、DNS、TCP 端口、代理检测、丢包率 |
| 系统资源 | 磁盘剩余空间（低于 1GB 报失败、低于 3GB 告警）、内存使用 |
| 安全软件 | EDR / 杀毒软件识别、文件读写监控检测 |
| 环境信息 | 系统信息、客户端版本、安装日志 |
| 崩溃日志 | 崩溃历史摘要 |
...
![诊断工具](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/self-check.zbT-hAIU.png)

### 2）日志在哪里查看

- **建议处理**：
	- **MAC：** 打开 **WorkBuddy**，点击顶部**帮助 \-\> 打开日志文件夹**，找到对应的日志 （.zip） 包。 ![](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/logmac.C9iZYyhZ.png)
	- **Windows：** 打开 **WorkBuddy**，在左上角**帮助**的下拉框中找到**打开日志目录**，其中的压缩包就是日志文件。 ![](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/logwin.afumbHyx.png)
...
![](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/%E6%84%8F%E8%A7%81%E5%8F%8D%E9%A6%88.B70drvin.png)## Bot 连接问题
...
![alt text](https://download.codebuddy.cn/web/docs/b59261153a07e588aebcf9cd16cba1bc45ad81f9/docs/static/%E5%8F%91%E7%A5%A8-1.wGOnDyAN.png)
