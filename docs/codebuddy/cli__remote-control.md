...
- **消息复制与时间**：用户消息右下方显示时间和复制按钮；助手正文在生成结束或停止后，在左下方显示复制按钮和时间。复制保留完整正文及 Markdown 格式，悬浮时间可查看完整日期；无可靠时间的历史消息不显示时间
- **长文本折叠**：已发送的用户文本超过 12 行时默认收起，点击“显示更多”或“收起”切换；折叠时仍可复制全文，图片和附件独立展示
