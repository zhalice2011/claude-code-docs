...
具备管理权限的 Agent CLI runtime 会在首轮投影提交后启动一次后台 reconcile。它负责 同步 Marketplace、补齐插件、迁移旧 cache、应用策略和清理孤儿状态；提交后通过同一条 串行事务队列提交 Plugin 活动快照，避免多轮管理和投影交叉。事务内会并行准备 MCP/LSP 与 Product 投影；Product 提交后，Command、Output Style 和 Agent 并行重建。 后台 reconcile 的活动快照没有变化时不重投下游；发生变化时也只刷新受影响的组件。首轮 投影接管管理所有权后，`enabledPlugins` 或 `extraKnownMarketplaces` 的持久化内容发生变化时， 同一长驻 runtime 会立即排队执行完整 configured reconcile；进程外直接编辑 settings 文件也由 ConfigWatchService 清缓存并补发同一个 settings change。多次连续改动通过 generation \+ single\-flight 收敛，不需要等到创建下一项任务才物化 cache。

`install` 与 `enable` 是不同语义。显式 `plugin install` 会同步解析依赖、写入版本化 `cache/<marketplace>/<plugin>/<version>`、提交 `installed_plugins.json`，并按标准默认写入 enabled state；`plugin enable` 只声明 desired state，即使该插件已能从 Marketplace 发现但尚未 安装，也不在 enable 操作中写 cache/registry。短命 `plugin enable` 命令退出后由下一次普通 Agent 启动 reconcile；长驻 Host 则由上述 settings change reconcile 立即接手物化。这与 Claude Code 的 install/enable/startup 边界一致。

`plugin update` 同样按版本化 cache 的不可变语义执行：Marketplace 中的声明版本与当前安装版本 相同时直接视为 up to date，不重写 cache、registry 或依赖状态；声明版本变化时写入新的版本目录并 原子切换 registry，旧 Session 继续引用旧目录。只有确实无法得到声明版本或 Git SHA 的 `unknown` cache 才允许尝试原位刷新；若仍有其他 Session marker，则保留旧内容及其 payload 元数据并延迟到 后续 update/reconcile，不会把安全延迟作为 HTTP 500 返回。这与 Claude Code 的 update/cache 边界一致。

长驻 Host 还可调用 `POST /api/v1/plugins/reconcile`，幂等物化 Product/settings 声明的 完整 configured state，并在物化后重建运行时投影。这个端点不接受宿主名、marketplace 名或 本地路径，因而不会形成 WorkBuddy 专用的管理分支；并发请求复用同一个底层 pass。HTTP waiter 有固定上限，超时或客户端断开只结束当前等待，不取消仍受跨进程锁保护的底层物化，后续请求仍可 加入或重试。严格聚合失败前已经原子提交的 marketplace/cache 会先投影为 partial/last\-good 状态。 同一路由带 `requiredPluginIds` body 时语义相反：它只投影并严格校验已经提交的 root 与依赖闭包， 不安装、不更新 Marketplace，也不写 cache/registry/settings。若 startup/settings producer 已经在 运行，校验可加入该既有 flight 一次再复查；它本身绝不会因为任务启动而创建新的管理写入。

普通 TUI、`--print`、headless、stream\-json、ACP 和 serve 调用共享同一条首轮边界：启动时从 已提交的 registry/cache 构建完整运行时投影，第一次真正 dispatch Agent 前等待该投影完成； 不会因协议模式不同插入前台 Marketplace 下载，也不会把下游投影延迟到 Session 控制请求。 单个 cache 缺失或加载失败只让对应插件缺席并记录插件错误，其他插件和 Agent 继续运行；缺失插件 由后台 reconcile 补齐并在提交后重投影。WorkBuddy 等宿主若声明本次 Session 的显式 required roots，应在配置/install/startup 阶段完成 cache 准备，并在拉起 worker 前只读校验；进程启动后 直接消费这份已提交状态，不再在 `session/new|load|resume` 前重复 reconcile。显式 `--no-plugin-management` 下只从已有 registry/cache 做完整投影，不产生管理写入。
