...
| 我想要 | 使用 |
| --- | --- |
| 仅加载你接触的代码的约定，而不是一个根文件覆盖每个子系统 | 按目录的 [CODEBUDDY.md 文件](#按目录分层-codebuddymd-文件) |
...
按目录的 `CODEBUDDY.md` 文件和 `.codebuddy/rules/` 下的[路径范围规则](./memory#使用-codebuddyrules-实现模块化规则)都允许你将指令定向到树的一部分。它们在文件位置和加载时间上有所不同。
...
- **在编辑前将计划保存到文件**：[先计划](./best-practices#先探索再规划后编码)并要求 CodeBuddy 将计划写入存储库中的 markdown 文件。长的跨包会话在进行中压缩其上下文，保存的计划在对话历史可能不会的地方幸存
