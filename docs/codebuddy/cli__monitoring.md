...
默认上报使用 CodeBuddy 自有命名（`codebuddy_code.*` span、`span.type` 等）。如需对接 agentLens（基于 OTel），通过 `OTEL_SEMCONV` 追加 `gen_ai.*` 语义约定字段（**只增不删**，保留全部原字段）：
...
**指定上报空间**：上报到 agentLens 时，还需通过 `OTEL_RESOURCE_ATTRIBUTES` 的 `tps.tenant.id` 指定上报空间，value 为 agentLens 空间密钥。该属性会随 resource 属性自动上报到对应空间，仅需设置环境变量、无需改代码。完整配置示例：

bash
```
export CODEBUDDY_CODE_ENABLE_TELEMETRY=1
export OTEL_EXPORTER_OTLP_ENDPOINT=<agentLens OTLP 接入地址>
export OTEL_SEMCONV=agentlens                              # 启用 gen_ai.* 语义约定
export OTEL_RESOURCE_ATTRIBUTES="tps.tenant.id=<空间密钥>"  # 指定 agentLens 上报空间
```
