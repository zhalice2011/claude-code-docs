...
发送给模型的内嵌图片统一为 JPEG、PNG 或 WebP，`image/jpg` 按实际图片内容归一化。BMP、TIFF 和 GIF 自动转为 PNG（GIF 仅保留首帧）；当前无法解码的 ICO、SVG、AVIF、HEIC、HEIF 会以文本占位，避免图片格式导致整轮请求失败。如需模型读取这些图片，请先转换为 PNG。远程图片链接仍由模型服务读取。
