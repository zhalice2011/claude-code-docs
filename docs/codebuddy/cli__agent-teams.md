...
查看成员时，即使所有 agent 已结束，底部仍保留 `main` 入口。输入框为空时按 `↓` 进入底栏，选中 `main` 后按 `Enter` 返回主会话；如果还有后台 shell 或 workflow，继续按 `↓` 可到达 agent 层。

成员结束后，也可以在空输入框按 `Esc` 返回主会话。处于 Vim 插入模式或历史搜索时，`Esc` 优先退出当前编辑模式；退出这些模式后，空输入框的 `Esc` 才返回主会话。成员仍在运行时，`Esc` 保持停止当前成员的行为；底栏选择模式中的 `Esc` 只取消选择。
