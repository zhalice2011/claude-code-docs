...
Skill 生成交付物（pptx、pdf、xlsx 等）时，通常是跑一个脚本把文件做出来、再上传到对象存储拿到链接。这个链接默认只是脚本的一行普通输出：CodeBuddy Code 无从知道它是一份交付物，所以在被其他 Agent 通过 A2A 协议调用时，调用方收不到这份产物。

脚本可以主动声明。每次执行 Bash 命令时，CodeBuddy Code 会为这次命令单独准备一个空文件，把它的路径放在环境变量 `$CODEBUDDY_ARTIFACTS` 里。脚本往这个文件追加一行 JSON，一行代表一个产物：

bash
```
echo '{"uri":"https://example.com/decks/q3.pptx","name":"Q3 业务回顾.pptx"}' >> "$CODEBUDDY_ARTIFACTS"
```
命令结束后，CodeBuddy Code 读走这个文件并删除它。脚本不需要创建、清理或猜测这个路径，也不需要在多次调用之间做隔离——每次命令拿到的都是一个新文件。

**声明只能走这个文件，标准输出不会被解析。** 早期版本曾约定在 stdout 打一行 `::codebuddy-artifact::` 标记，该写法已经移除：现在打这样一行只是普通输出，既不会产出 artifact，也不会有告警。仍在用旧写法的脚本请改成上面的 `>> "$CODEBUDDY_ARTIFACTS"`。

之所以不从标准输出读，是因为它同时承载脚本自己说的话和它读出来转述的内容：`cat` 一个正好包含该标记的日志文件会被误当成真声明，而声明里如果写的是本地路径，文件内容就会被读出来发给调用方。带外文件没有这个顾虑——`cat` 写到 stdout，永远碰不到那个文件，所以本地路径只在这条通道里被接受。

### 字段

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `uri` | 是 | 远程链接，或本地绝对路径（必须在项目目录内，见下） |
| `name` | 否 | 展示名称，默认取 `uri` 的文件名部分 |
| `mimeType` | 否 | 内容类型，默认按扩展名推断 |
| `description` | 否 | 一句话说明，一次运行产出多个文件时有用 |
| `sizeBytes` | 否 | 字节大小，便于调用方决定是否下载 |

建议显式给出 `mimeType`。带签名的远程链接往往拖着很长的查询串，扩展名推断会失效。

### 生效范围

产出 A2A artifact 只发生在 CodeBuddy Code **作为被调方、通过 A2A 协议被其他 Agent 调用**时：声明会变成 `kind: "file"` 的 artifact 下发给调用方，远程链接以 URL 形式给出，本地路径按文件内容处理。

在普通命令行会话或 ACP 客户端里，声明会被正常读取，但没有下游消费者，因此不会产生额外效果。也就是说，给 skill 加声明不会破坏它在本地的使用方式。

### 行为细节

- 一个产物一行，多个产物追加多行，单次命令最多识别 64 个，超出的会被忽略并留下告警。
- 单行 JSON 上限 8192 字节。声明是元数据，不要把文件内容本身写进来。
- 声明的本地路径必须在**项目目录之内**才会被读取内容并内联给调用方；指向项目外的路径（例如 `~/.ssh/id_rsa`）会被拒绝，产物退化成一个纯引用。远程链接不受此限制。
- 写坏的行会被跳过并在日志里留下一条告警，不影响同一文件里其他正确的行，也不影响命令的成败判定。
- 声明不经过标准输出，因此输出量很大、正文被截断或转存到文件都不影响声明。
- `$CODEBUDDY_ARTIFACTS` 在沙箱下也可写，不会触发额外的授权弹窗。
- 命令因沙箱升级被重跑时，前一次的声明会作废，脚本重跑时重新追加即可。
- `run_in_background: true` 启动的后台命令可以写这个文件，但结果在命令刚启动时就返回了，模型之后通过 TaskOutput 取输出，那条链路目前还不读这个通道，因此声明**暂不会**变成 artifact（真写了会在日志里留告警）。需要产出 artifact 的 skill 请用前台执行。
