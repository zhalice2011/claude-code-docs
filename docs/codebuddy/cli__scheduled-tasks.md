...
`/loop` 任务每次触发执行完毕后，AI 会根据本轮结果自主判断下一轮应该：提前触发、延后触发、维持原定间隔，或者直接结束循环。例如"检查一下流水线是否跑完"这类任务，如果流水线还在跑，AI 可能会决定缩短下一轮间隔以更快得到结果；如果流水线已经跑完，AI 可能会决定结束循环。

这个调整是自动进行的，不需要你手动干预；循环最长仍然只会持续 3 天，随时可以按[取消任务](#取消任务)一节手动结束。
