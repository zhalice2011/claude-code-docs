...
安全中心统一管理与安全、运行时相关的配置。除了上面两种权限模式，你还可以在这里进一步收紧 Agent 的行为边界。

### 程序黑名单

在**命令安全**中设置程序黑名单，被列入的程序将无法被 Agent 及其调用的子程序启动。

- 适合封禁你不希望 AI 触碰的程序
- 命中后相关调用会被直接阻止，不会再向你询问

### 文件读写规则

在**强制审批黑名单**中，可以为敏感目录设置拦截规则：
...
- 只访问文件名、不读取内容的工具（如 `find`）不会触发弹窗
- 读取文件内容的工具（如 `grep`）同样受读规则管控
- 默认规则已包含对 `ssh` 等敏感数据目录的读保护
...
WorkBuddy 内置恶意域名库，在沙箱内发起网络访问（脚本、命令行等）或抓取网页前，会先检查目标域名，命中恶意域名即拒绝访问。

- 判断全部在**本地**完成，每次访问不会向后台发起请求
- 域名库每天更新，更新失败时继续使用上一版本
- 覆盖沙箱内的网络访问与网页抓取；不覆盖插件内部的网络访问与搜索服务